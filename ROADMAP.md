...
| **planned** | Free-floating canvas links: each with its own `{x,y}` (no strip UI); **Add link** beside Save in [`EditModeToolbar`](./src/components/editor/EditModeToolbar.tsx); list view shows them under a synthetic **Ungrouped** row—extend [`AppState`](./src/types/index.ts), [`Canvas`](./src/components/canvas/Canvas.tsx), [`ListView`](./src/components/list/ListView.tsx), palette, and [`background.ts`](./src/background.ts). |
